...
int CRijndael::Init(Mode mode, Direction dir, const UINT8 *key, KeyLength keyLen, const UINT8 *initVector)
{
	m_state = Invalid; // Not initialized yet
	...
	// And the direction
	if((dir != EncryptDir) && (dir != DecryptDir)) return RIJNDAEL_UNSUPPORTED_DIRECTION;
	m_direction = dir;

	// Allow to set an init vector
	if(initVector != NULL)
	{
		for(int iv = 0; iv < RD_MAX_IV_SIZE; iv++)
			m_initVector[iv] = initVector[iv];
	}
	else
	{
		for(int iv = 0; iv < RD_MAX_IV_SIZE; iv++)
	...
	if(uKeyLenInBytes == 0) return RIJNDAEL_UNSUPPORTED_KEY_LENGTH;
	...
	UINT8 keyMatrix[RD_MAX_KEY_COLUMNS][4];

	for(UINT32 i = 0; i < uKeyLenInBytes; i++)
		keyMatrix[i >> 2][i & 3] = key[i];

	KeySched(keyMatrix);
	if(m_direction == DecryptDir) KeyEncToDec();
	m_state = Valid;

	return RIJNDAEL_SUCCESS;
}

int CRijndael::BlockEncrypt(const UINT8 *input, int inputLen, UINT8 *outBuffer)
{
	int i, k, numBlocks;
	UINT8 block[16], iv[4][4];

	if(m_state != Valid) return RIJNDAEL_NOT_INITIALIZED;
	if(m_direction != EncryptDir) return RIJNDAEL_BAD_DIRECTION;
...
int CRijndael::PadEncrypt(const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
	int i, numBlocks, padLen;
	UINT8 block[16], *iv;

	if(m_state != Valid) return RIJNDAEL_NOT_INITIALIZED;
	if(m_direction != EncryptDir) return RIJNDAEL_NOT_INITIALIZED;
...
int CRijndael::BlockDecrypt(const UINT8 *input, int inputLen, UINT8 *outBuffer)
{
	int i, k, numBlocks;
	UINT8 block[16], iv[4][4];

	if(m_state != Valid) return RIJNDAEL_NOT_INITIALIZED;
	if((m_mode != CFB1) && (m_direction == EncryptDir)) return RIJNDAEL_BAD_DIRECTION;
...
int CRijndael::PadDecrypt(const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
	int i, numBlocks, padLen;
	UINT8 block[16];
	UINT32 iv[4];
	...
	if(m_state != Valid) return RIJNDAEL_NOT_INITIALIZED;
	if(m_direction != DecryptDir) return RIJNDAEL_BAD_DIRECTION;
...
void CRijndael::KeySched(UINT8 key[RD_MAX_KEY_COLUMNS][4])
{
	int j, rconpointer = 0;
	...
	UINT8 tempKey[RD_MAX_KEY_COLUMNS][4];
	...
	{
		tempKey[0][0] ^= g_S[tempKey[uKeyColumns-1][1]];
		tempKey[0][1] ^= g_S[tempKey[uKeyColumns-1][2]];
		tempKey[0][2] ^= g_S[tempKey[uKeyColumns-1][3]];
		tempKey[0][3] ^= g_S[tempKey[uKeyColumns-1][0]];
		tempKey[0][0] ^= g_RCON[rconpointer++];
		...
		else
		{
			...
			tempKey[uKeyColumns/2][0] ^= g_S[tempKey[uKeyColumns/2 - 1][0]];
			tempKey[uKeyColumns/2][1] ^= g_S[tempKey[uKeyColumns/2 - 1][1]];
			tempKey[uKeyColumns/2][2] ^= g_S[tempKey[uKeyColumns/2 - 1][2]];
			tempKey[uKeyColumns/2][3] ^= g_S[tempKey[uKeyColumns/2 - 1][3]];
...
void CRijndael::KeyEncToDec()
{
	...
	{
		w = m_expandedKey[r][0];
		*((UINT32 *)w) = *((UINT32 *)g_U1[w[0]]) ^ *((UINT32 *)g_U2[w[1]]) ^ *((UINT32 *)g_U3[w[2]]) ^ *((UINT32 *)g_U4[w[3]]);
		w = m_expandedKey[r][1];
		*((UINT32 *)w) = *((UINT32 *)g_U1[w[0]]) ^ *((UINT32 *)g_U2[w[1]]) ^ *((UINT32 *)g_U3[w[2]]) ^ *((UINT32 *)g_U4[w[3]]);
		w = m_expandedKey[r][2];
		*((UINT32 *)w) = *((UINT32 *)g_U1[w[0]]) ^ *((UINT32 *)g_U2[w[1]]) ^ *((UINT32 *)g_U3[w[2]]) ^ *((UINT32 *)g_U4[w[3]]);
...
void CRijndael::Encrypt(const UINT8 a[16], UINT8 b[16])
{
	int r;
	UINT8 temp[4][4];
    ...
    *((UINT32 *)(b    )) = *((UINT32 *)g_T1[temp[0][0]])
						^ *((UINT32 *)g_T2[temp[1][1]])
						^ *((UINT32 *)g_T3[temp[2][2]])
						^ *((UINT32 *)g_T4[temp[3][3]]);
    *((UINT32 *)(b + 4)) = *((UINT32 *)g_T1[temp[1][0]])
						^ *((UINT32 *)g_T2[temp[2][1]])
						^ *((UINT32 *)g_T3[temp[3][2]])
						^ *((UINT32 *)g_T4[temp[0][3]]);
    *((UINT32 *)(b + 8)) = *((UINT32 *)g_T1[temp[2][0]])
						^ *((UINT32 *)g_T2[temp[3][1]])
						^ *((UINT32 *)g_T3[temp[0][2]])
						^ *((UINT32 *)g_T4[temp[1][3]]);
    *((UINT32 *)(b +12)) = *((UINT32 *)g_T1[temp[3][0]])
						^ *((UINT32 *)g_T2[temp[0][1]])
						^ *((UINT32 *)g_T3[temp[1][2]])
						^ *((UINT32 *)g_T4[temp[2][3]]);
	...
	*((UINT32 *)temp[3]) = *((UINT32 *)(b+12)) ^ *((UINT32 *)m_expandedKey[m_uRounds-1][3]);
	...
	b[ 8] = g_T1[temp[2][0]][1];
	b[ 9] = g_T1[temp[3][1]][1];
	b[10] = g_T1[temp[0][2]][1];
	b[11] = g_T1[temp[1][3]][1];
	b[12] = g_T1[temp[3][0]][1];
	b[13] = g_T1[temp[0][1]][1];
	b[14] = g_T1[temp[1][2]][1];
	b[15] = g_T1[temp[2][3]][1];
...
void CRijndael::Decrypt(const UINT8 a[16], UINT8 b[16])
{
	int r;
	UINT8 temp[4][4];
    ...
    *((UINT32 *)(b   )) = *((UINT32 *)g_T5[temp[0][0]])
           ^ *((UINT32 *)g_T6[temp[3][1]])
           ^ *((UINT32 *)g_T7[temp[2][2]])
           ^ *((UINT32 *)g_T8[temp[1][3]]);
	*((UINT32 *)(b+ 4)) = *((UINT32 *)g_T5[temp[1][0]])
           ^ *((UINT32 *)g_T6[temp[0][1]])
           ^ *((UINT32 *)g_T7[temp[3][2]])
           ^ *((UINT32 *)g_T8[temp[2][3]]);
	*((UINT32 *)(b+ 8)) = *((UINT32 *)g_T5[temp[2][0]])
           ^ *((UINT32 *)g_T6[temp[1][1]])
           ^ *((UINT32 *)g_T7[temp[0][2]])
           ^ *((UINT32 *)g_T8[temp[3][3]]);
	*((UINT32 *)(b+12)) = *((UINT32 *)g_T5[temp[3][0]])
           ^ *((UINT32 *)g_T6[temp[2][1]])
           ^ *((UINT32 *)g_T7[temp[1][2]])
           ^ *((UINT32 *)g_T8[temp[0][3]]);
