...
void CWzComboBox::DrawIconString(LPDRAWITEMSTRUCT lpDIS, BOOL bselected)
{
	...
	if (m_pImgList->GetImageCount() > 0)//has icon attach
	{
		POINT pt;
		CRect rcIcon;
...
//#endif
		...
		if(bselected)
		{
			...
			// pDC->Draw3dRect(rcIcon,RGB(224,224,224),RGB(128,128,128));
		}
...
{
	int idx = -1;
	if(wParentIdx == WZ_ROOT_INDEX)//if root node,just Add to the end of String List
	{
		...
		pData->wParentOriginIdx = wParentIdx;
...
void CWzComboBox::SetXImageList(CImageList *pImgList)
{
	m_pImgList = pImgList;
}
