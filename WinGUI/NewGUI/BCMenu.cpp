...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		size_t sizeMenuTextBuffer = sizeof(wchar_t)*(wcslen(szWideString)+1);
		m_szMenuText = new wchar_t[sizeMenuTextBuffer];
		if (m_szMenuText)
			wcscpy_s(m_szMenuText,sizeMenuTextBuffer,szWideString);
    }
	else
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		nLen = 0;
		size_t sizeCaptionBufferLen = wcslen((wchar_t *)pTp)+1;
		szCaption=new wchar_t[sizeCaptionBufferLen];
		wcscpy_s(szCaption,sizeCaptionBufferLen,(wchar_t *)pTp);
