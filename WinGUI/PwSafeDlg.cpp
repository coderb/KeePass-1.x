...
BOOL CPwSafeDlg::OnInitDialog()
{
	...
	UINT i, uState, uID, uLastID = (UINT)(-1);
	CString str;
	...
	if(szTemp[0] != 0)
		NewGUI_SetBannerColors(static_cast<DWORD>(_ttol(szTemp)), DWORD_MAX);
	cConfig.Get(PWMKEY_BANNERCOLOREND, szTemp);
	if(szTemp[0] != 0)
		NewGUI_SetBannerColors(DWORD_MAX, static_cast<DWORD>(_ttol(szTemp)));
	...
	m_tip.AddTool(&m_cQuickFind, TRL("Type to search the database"));
	m_tip.Activate(TRUE);
	...
	if((m_bMiniMode == TRUE) && (m_bFileOpen == FALSE) && (m_bLocked == FALSE))
	{
		if((bTriedOpenDb == FALSE) && (m_bUnintrusiveMiniMode == FALSE))
		{
			...
			{
				if(m_bUnintrusiveMiniMode == FALSE) OnFileExit();
			}
...
void CPwSafeDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if(((nType == SIZE_RESTORED) || (nType == SIZE_MAXIMIZED)) &&
		(bWasInvisible == TRUE))
	{
		ChangeLockStateOnWindowChange(FALSE);
	}
	else if(nType == SIZE_MINIMIZED)
	{
		if(m_bLockOnMinimize == TRUE)
		{
			ChangeLockStateOnWindowChange(TRUE); // Lock
...
void CPwSafeDlg::_DeleteTemporaryFiles()
{
	...
	{
		if(SecureDeleteFile(m_vTempFiles[i].c_str()) == FALSE)
...
DWORD CPwSafeDlg::GetSelectedGroupId()
{
	...
	return (DWORD)(m_cGroups.GetItemData(h));
}
...
void CPwSafeDlg::UpdatePasswordList()
{
	...
	if(m_bFileOpen == FALSE) { m_cList.DeleteAllItems(); return; }

	const DWORD dwGroupId = GetSelectedGroupId();
	if(dwGroupId == DWORD_MAX) { m_cList.DeleteAllItems(); return; }
...
void CPwSafeDlg::_List_SetEntry(DWORD dwInsertPos, PW_ENTRY *pwe, BOOL bIsNewEntry, PW_TIME *ptNow)
{
	...
	BOOL bIsTAN = CPwUtil::IsTANEntry(pwe);
...
void CPwSafeDlg::RefreshPasswordList()
{
	...
	DWORD j = 0;
	for(DWORD i = 0; i < (DWORD)m_cList.GetItemCount(); i++)
	{
		LV_ITEM lvi;
		ZeroMemory(&lvi, sizeof(LV_ITEM));
		lvi.iItem = (int)i;
...
void CPwSafeDlg::OnPwlistAdd()
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		{
			if(dlg.m_strAttachment.Left(static_cast<int>(_tcslen(PWS_NEW_ATTACHMENT))) ==
				CString(PWS_NEW_ATTACHMENT))
			{
				CPwUtil::AttachFileAsBinaryData(pNew,
					dlg.m_strAttachment.Right(dlg.m_strAttachment.GetLength() -
					(int)_tcslen(PWS_NEW_ATTACHMENT)));
			}
		}
					...
					if(m_nLockCountdown == 0)
					{
						if(m_bExitInsteadOfLockAT == FALSE)
						{
							...
							if((m_bLocked == TRUE) && (m_bMinimized == FALSE) &&
								(m_bTrayed == FALSE))
							{
								SetViewHideState(FALSE, TRUE); // Minimize/tray
							}
						}
						else OnFileExit();
					}
...
void CPwSafeDlg::OnDblclkPwlist(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	BOOL bIsTAN = CPwUtil::IsTANEntry(p);
...
void CPwSafeDlg::ParseAndOpenURLWithEntryInfo(LPCTSTR lpURL, PW_ENTRY *pEntry)
{
	if((pEntry != NULL) && (lpURL != NULL))
	{
		...
		if(strURL.GetLength() != 0)
		{
			...
			WU_FillPlaceholders(&strURL);

			const BOOL bCmdQuotes = (strURL.Left(6) == _T("cmd://")) ? TRUE : FALSE;

			m_mgr.UnlockEntryPassword(pEntry);
			ParseURL(&strURL, pEntry, FALSE, bCmdQuotes);
			if(bParseTwoTimes == TRUE) ParseURL(&strURL, pEntry, FALSE, bCmdQuotes);
			m_mgr.LockEntryPassword(pEntry);
...
void CPwSafeDlg::OnPwlistVisitUrl()
{
	...
	else // m_bCopyURLs == TRUE
	{
		...
		// FixURL(&strURL);
		m_mgr.UnlockEntryPassword(p);
		ParseURL(&strURL, p, FALSE, (strURL.Left(6) == _T("cmd://")) ? TRUE : FALSE);
		m_mgr.LockEntryPassword(p);
...
BOOL CPwSafeDlg::_ChangeMasterKey(CPwManager *pDbMgr, BOOL bCreateNew)
{
	...
	if(nErrCode != PWE_SUCCESS)
	{
		if(dlg.m_bKeyFile == TRUE)
		{
			...
			int nMsg = MessageBox(strMsg, TRL("Overwrite?"), MB_ICONQUESTION |
			...
			if(nMsg == IDYES)
			{
				...
				{
					dlg.FreePasswords();
					MessageBox(CPwUtil::FormatError(nErrCode2, 0),
		...
		{
			dlg.FreePasswords();
			MessageBox(CPwUtil::FormatError(nErrCode, 0),
				PWM_PRODUCT_NAME_SHORT, MB_OK | MB_ICONWARNING);
			m_bDisplayDialog = FALSE; return FALSE;
		}
	}
	...
	{
		CString str;
		str = TRL("The composite master key has been changed!");
		str += _T("\r\n\r\n");
...
void CPwSafeDlg::_OpenDatabase(CPwManager *pDbMgr, const TCHAR *pszFile, const TCHAR *pszPassword, const TCHAR *pszKeyFile, BOOL bOpenLocked, LPCTSTR lpPreSelectPath, BOOL bIgnoreCorrupted)
{
	...
	int nOpenAttempts, nAllowedAttempts, nErr;
	const TCHAR *pSuffix = _T("");
	...
	strFilter = TRL("KeePass Password Safe Files");
	...
	if((pszFile != NULL) || (nRet == IDOK))
	{
		...
		else
		{
			...
			for(nOpenAttempts = 0; nOpenAttempts < nAllowedAttempts; nOpenAttempts++)
			{
				...
				if(nErr != PWE_SUCCESS)
				{
					MessageBox(CPwUtil::FormatError(nErr, 0),
						PWM_PRODUCT_NAME_SHORT, MB_OK | MB_ICONWARNING);
					continue;
				}
...
void CPwSafeDlg::OnFileSaveAs()
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		if(nErr != PWE_SUCCESS)
		{
			MessageBox(CPwUtil::FormatError(nErr, 0), PWM_PRODUCT_NAME_SHORT,
...
void CPwSafeDlg::OnFileClose()
{
	...
	if((m_bFileOpen == TRUE) && (m_bModified == TRUE))
	{
		...
		{
			CString str = TRL("The current file has been modified.");
			str += _T("\r\n\r\n");
	...
	if((m_bMenuExit == TRUE) || (m_bMiniMode == FALSE))
		ClearClipboardIfOwner();
...
void CPwSafeDlg::OnSafeRemoveGroup()
{
	...
	if(nRes == IDYES)
	{
		INT_PTR nCount = aGroupIds.GetSize();
	...
	CPwExport *pPwExport, BOOL bPrinting)
{
	...
	*pOptions = pPwExport->m_aDefaults[pPwExport->m_nFormat];
	...
	if(bPrinting == TRUE) { ASSERT(n == (LCL_CHKOPT_PARAM_COUNT - 2)); }
...
void CPwSafeDlg::_PrintGroup(DWORD dwGroupId)
{
	...
	BOOL bRet = cExp.ExportGroup(tszFile.c_str(), dwGroupId, &pwo, NULL);
	if(bRet == FALSE)
	{
...
void CPwSafeDlg::_Find(DWORD dwFindGroupId)
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		while(1)
		{
			const DWORD dw = m_mgr.Find(dlg.m_strFind, dlg.m_bCaseSensitive,
				dwFlags, cnt);
		...
		{
			m_cList.SetItemState(0, LVIS_SELECTED | LVIS_FOCUSED,
...
void CPwSafeDlg::OnFileLock()
{
	...
	else
	{
		...
		if(m_bFileOpen == FALSE)
		{
			...
			// MessageBox(TRL("Workspace cannot be unlocked!"), PWM_PRODUCT_NAME_SHORT, MB_ICONINFORMATION | MB_OK);
...
void CPwSafeDlg::OnImportCsv()
{
	...
	if(nRet == IDOK)
	{
		...
		{
			CString str = TRL("An error occured while importing the file. File cannot be imported.");
			str += _T("\r\n\r\n");
...
CString CPwSafeDlg::_MakeRtfString(LPCTSTR lptString)
{
	...
	for(int i = 0; i < static_cast<int>(_tcslen(lptString)); ++i)
	{
		const TCHAR tch = lptString[i];
...
void CPwSafeDlg::ShowEntryDetails(PW_ENTRY *p)
{
	...
	crURL.cpMin += static_cast<LONG>(_tcslen(TRL("Group:")) + 1 +
		_tcslen(pg->pszGroupName));
...
void CPwSafeDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if(m_hDraggingGroup != NULL)
	{
		if(m_bCanDragGroup == TRUE)
		{
			...
			{
				DWORD dwDragGroupId = (DWORD)m_cGroups.GetItemData(m_hDraggingGroup);
				...
				{
					DWORD dwDragToGroupId = (DWORD)m_cGroups.GetItemData(hItemDropTo);
...
void CPwSafeDlg::OnFilePrintPreview()
{
	...
	BOOL bRet = cExp.ExportGroup(tszFile.c_str(), DWORD_MAX, &pwo, NULL); // Export all: set group ID to DWORD_MAX
...
BOOL CPwSafeDlg::IsUnsafeAllowed(HWND hWndParent)
{
	...
	::MessageBox(hWndParent, (LPCTSTR)str, TRL("Stop"), MB_ICONWARNING | MB_OK);
...
void CPwSafeDlg::OnFileShowDbInfo()
{
	...
	dlg.m_strDefaultUserName = (LPCTSTR)strName; // Copy
...
void CPwSafeDlg::_ShowExpiredEntries(BOOL bShowIfNone, BOOL bShowExpired, BOOL bShowSoonToExpire)
{
	...
	if(bShowIfNone == FALSE)
	{
		...
		for(DWORD i = 0; i < m_mgr.GetNumberOfEntries(); i++)
		{
			...
			{
				dwDate = ((DWORD)pe->tExpire.shYear << 16) | ((DWORD)pe->tExpire.btMonth << 8) |
					((DWORD)pe->tExpire.btDay & 0xFF);
...
void CPwSafeDlg::_HandleEntryDrop(DWORD dwDropType, HTREEITEM hTreeItem)
{
	...
	for(DWORD i = 0; i < (DWORD)m_cList.GetItemCount(); i++)
	{
		const UINT uState = m_cList.GetItemState((int)i, LVIS_SELECTED);
...
void CPwSafeDlg::_AutoType(PW_ENTRY *pEntry, BOOL bLoseFocus, DWORD dwAutoTypeSeq)
{
	...
	if(bLoseFocus == TRUE)
	{
		...
		{
			BOOL bPrevIgnoreSizeEvent = m_bIgnoreSizeEvent;
			m_bIgnoreSizeEvent = TRUE;
...
void CPwSafeDlg::OnUpdatePluginMessage(CCmdUI* pCmdUI)
{
	for(int i = 0; i < (int)CPluginManager::Instance().m_plugins.size(); ++i)
	{
		for(int j = 0; j < (int)CPluginManager::Instance().m_plugins[i].info.dwNumCommands; ++j)
		{
			const KP_MENU_ITEM *pMenuItem =
				&CPluginManager::Instance().m_plugins[i].info.pMenuItems[j];
...
void CPwSafeDlg::BuildPluginMenu()
{
	...
	BCMenu *psub = NewGUI_GetBCMenu(m_menu.GetSubMenu((TCHAR *)TRL("&Tools")));
	CString strItem;
...
BOOL CPwSafeDlg::RegisterGlobalHotKey(int nHotKeyID, DWORD dwHotKey, BOOL bReleasePrevious, BOOL bMessageBoxOnFail)
{
	...
	if(b == FALSE)
	{
		...
		if(bMessageBoxOnFail == TRUE)
		{
			...
			str += TRL("You can choose a different hot key combination in 'Tools - Options - Setup'.");
...
LRESULT CPwSafeDlg::OnHotKey(WPARAM wParam, LPARAM lParam)
{
	...
	if(wParam == HOTKEYID_AUTOTYPE)
	{
		...
		ASSERT(pszWindow != NULL); if(pszWindow == NULL) { m_bGlobalAutoTypePending = FALSE; return 0; }
		...
		if(dwMatchingEntriesCount != 0)
		{
			...
			if(pe != NULL)
			{
				...
				_AutoType(pe, FALSE, dwWindowFieldSeqFound);
			}
		}
...
void CPwSafeDlg::ExportGroupToKeePass(DWORD dwGroupId)
{
	...
	const int nUse = MessageBox(strText, PWM_PRODUCT_NAME_SHORT,
...
{
	if(m_bLocked == FALSE) m_systray.SetIcon(m_hTrayIconNormal);
...
void CPwSafeDlg::ChangeLockStateOnWindowChange(BOOL bLocked)
{
	...
	const CPrivateConfigEx& cConfig)
{
	...
	else
	{
		...
		m_strFile += _T("\\Database");
		for(DWORD dwTest = 0; ; ++dwTest)
		{
			CString strCan;
...
void CPwSafeDlg::SetTrayState(BOOL bMinimizeToTray)
{
	if(bMinimizeToTray == TRUE)
	{
		...
		if(m_bLockOnMinimize == TRUE)
			ChangeLockStateOnWindowChange(TRUE);
	}
...
void CPwSafeDlg::OnQuickFindSelChange()
{
	...
	int iSel = m_cQuickFind.GetCurSel();
	if(iSel == CB_ERR)
	{
		ASSERT(FALSE);
		m_bBlockQuickFindSelChange = FALSE;
